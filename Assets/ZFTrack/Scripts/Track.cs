...
namespace ZenFulcrum.Track
{
	...
	#region Constants, static, types
        ...
        public class SpeedAndForce
        {
            ...
            public int IsActive(float forwardSpeed)
            {
                ...
                if (direction == ActDirection.Both)
                {
                    ...
                    if (forwardSpeed + forwardnessTolerance >= 0)
                        return 1;
                    else
                        return -1;
                }
        ...
        public float curveStartStrength = DefaultCurveStrength,
            curveEndStrength = DefaultCurveStrength;
        ...
        public void Awake()
        {
            ...
            if (acceleration == null)
                acceleration = new SpeedAndForce();
            if (brakes == null)
                brakes = new SpeedAndForce();
        }
        ...
        public void OnDestroy()
        {
            ...
            if (NextTrack)
                NextTrack.Dirty = true;
            if (PrevTrack)
                PrevTrack.Dirty = true;
        ...
        protected Mesh TryToFindGeneratedMesh()
        {
            ...
            if (
                GameObject
                    ...
                    .Any(x => x != mf && x.sharedMesh == currentMesh)
            )
            {
                return null;
            }
        ...
        {
            get { return (transform.position + TrackAbsoluteEnd.position) * .5f; }
        }
        ...
        public TrackCurve Curve
        {
            get
            {
                if (_curve == null)
                {
                    ...
                    {
                        Debug.LogError(
                            "This track's end is coincident with the start, moving start and end apart.",
                            gameObject
                        );
                        this.TrackEnd = new SimpleTransform(
                            new Vector3(0, 0, 1),
        ...
        public Track PrevTrack
        {
            ...
            set
            {
                ...
                if (oldPrev)
                    oldPrev.RefreshIgnoredColliders();

                if ((bool)oldPrev != (bool)_prevTrack)
                    Dirty = true;
            }
        }
        public Track NextTrack
        {
            ...
            set
            {
                ...
                RefreshIgnoredColliders(); //ignore collisions with adjacent track again
                if (oldNext)
                    oldNext.RefreshIgnoredColliders();

                //If the need for an end cap has changed, regen the mesh.
                if ((bool)oldNext != (bool)_nextTrack)
                    Dirty = true;
            }
        }
        ...
        public bool IsStartNearest(Vector3 pos)
        {
            return Vector3.Distance(transform.position, pos)
                < Vector3.Distance(TrackAbsoluteEnd.position, pos);
        }
	...
	#region Generated Mesh and Items
        ...
        protected void GenerateCollider()
        {
            ...
            if (GetComponentInParent<Rigidbody>() && !meshCollider.convex)
            {
                ...
                Debug.LogWarning(
                    "To allow approximate collisions on this track, mark the mesh collider as convex",
                    meshCollider
                );
            }
#endif
        ...
        protected void GenerateMesh()
        {
            ...
            {
                Debug.LogWarning(
                    "Trying to update a mesh on a track marked as static. (A track's mesh can't be modified if it's static.)",
                    this
                );
            }
#endif
        ...
        public Track GetNext(Track prev)
        {
            ...
            else
            {
                ...
                Debug.LogWarning(
                    "Invalid: called Track.getNext with a \"previous\" track that's not actually connected",
                    this
                );
                return null;
            }
        ...
        protected void RefreshIgnoredColliders()
        {
            foreach (var other in ignoredColliders)
                __ignoreCollisions(other, true);
            if (NextTrack)
                foreach (var other in NextTrack.ignoredColliders)
                    __ignoreCollisions(other, true);
            if (PrevTrack)
                foreach (var other in PrevTrack.ignoredColliders)
                    __ignoreCollisions(other, true);
        }
        ...
        {
            if (!a || !a.enabled || !b || !b.enabled)
                return;

            var aMC = a as MeshCollider;
            if (aMC && (!aMC.sharedMesh || aMC.sharedMesh.vertexCount == 0))
                return;

            var bMC = a as MeshCollider;
            if (bMC && (!bMC.sharedMesh || bMC.sharedMesh.vertexCount == 0))
                return;
