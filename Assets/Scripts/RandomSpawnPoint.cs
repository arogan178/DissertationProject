...
public class RandomSpawnPoint : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (randomGameController.currentState == 0 || trackSetup.fastModeOn)
        {
            if (enemies.Length < 8)
            {
                for (int i = 0; i < (10 - enemies.Length); i++)
                {
                    ...
                    int targetPointIdx = Random.Range(0, followPoints.Length);
        ...
        else if (randomGameController.currentState == 1 || !trackSetup.fastModeOn)
        {
            if (enemies != null)
            {
